#!/usr/bin/env bash
# Update the version and hashes in home/opencode/package.nix.
# Usage: ./update-hashes.sh [VERSION]
#   VERSION  optional, e.g. 1.14.30 (without the leading 'v').
...
HASH_X86_LINUX=$(sri  "${BASE_URL}/v${VERSION}/opencode-linux-x64.tar.gz")
HASH_ARM_LINUX=$(sri  "${BASE_URL}/v${VERSION}/opencode-linux-arm64.tar.gz")
HASH_ARM_DARWIN=$(sri "${BASE_URL}/v${VERSION}/opencode-darwin-arm64.zip")
...
sed -i \
  "/opencode-linux-x64\.tar\.gz/{n; s|hash = \"[^\"]*\";|hash = \"${HASH_X86_LINUX}\";|}" \
  "$PACKAGE_NIX"

sed -i \
  "/opencode-linux-arm64\.tar\.gz/{n; s|hash = \"[^\"]*\";|hash = \"${HASH_ARM_LINUX}\";|}" \
  "$PACKAGE_NIX"

sed -i \
  "/opencode-darwin-arm64\.zip/{n; s|hash = \"[^\"]*\";|hash = \"${HASH_ARM_DARWIN}\";|}" \
  "$PACKAGE_NIX"
