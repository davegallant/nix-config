...
HASH_X86_LINUX=$(sri  "${BASE_URL}/v${VERSION}/pi-linux-x64.tar.gz")
HASH_ARM_LINUX=$(sri  "${BASE_URL}/v${VERSION}/pi-linux-arm64.tar.gz")
HASH_ARM_DARWIN=$(sri "${BASE_URL}/v${VERSION}/pi-darwin-arm64.tar.gz")
...
sed -i \
  "/pi-linux-x64\.tar\.gz/{n; s|hash = \"[^\"]*\";|hash = \"${HASH_X86_LINUX}\";|}" \
  "$PACKAGE_NIX"

sed -i \
  "/pi-linux-arm64\.tar\.gz/{n; s|hash = \"[^\"]*\";|hash = \"${HASH_ARM_LINUX}\";|}" \
  "$PACKAGE_NIX"

sed -i \
  "/pi-darwin-arm64\.tar\.gz/{n; s|hash = \"[^\"]*\";|hash = \"${HASH_ARM_DARWIN}\";|}" \
  "$PACKAGE_NIX"
